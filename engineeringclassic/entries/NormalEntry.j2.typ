...
{% set github_label = entry.date_and_location_column.split(')[', 1)[1].rsplit(']', 1)[0] %}
    #box[#link("{{ github_url }}", icon: false, if-underline: false, if-color: true)[#connection-with-icon("github")[{{ github_label }}]]]

{% else %}
{% for line in entry.date_and_location_column.splitlines() %}
{% if "github.com" in line and "#link(" in line %}
    #box[{{ line|replace('if-color: false', 'if-color: true')|indent(9) }}]
